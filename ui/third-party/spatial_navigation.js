...
;(function($) {
  ...
  var GlobalConfig = {
    ...
    restrict: 'none', // 'self-first', 'self-only', 'none'
    tabIndexIgnoreList:
  ...
  function focusNext(direction, currentFocusedElement, currentSectionId) {
    ...
    if (next) {
      ...
      if (currentSectionId != nextSectionId) {
        ...
        console.log(nextSectionId, direction);
      return focusElement(next, nextSectionId, direction);
  ...
  function onFocus(evt) {
    ...
    console.log("I AM FOCUSED ON:", target.className);
    querySliderItems();
    //target.style.outline = "thick solid #6baee6";
    //target.scrollTo(target.getBoundingClientRect().y, target.getBoundingClientRect().y);
    ...
    init: function() {
      if (!_ready) {
        ...
        window.addEventListener('keyup', onKeyUp);
                window.addEventListener('keyup', onKeyRight);
